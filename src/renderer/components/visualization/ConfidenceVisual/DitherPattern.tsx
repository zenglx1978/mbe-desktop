...
/**
 * Dithering 纹理包装器 — 给任意内容加上置信度纹理
 */
export function DitherWrapper({
  grade,
  dark = false,
  children,
  className = '',
  ...
  const { className: ditherCls, style } = getDitherStyle(grade, dark)
  return (
    <div className={`rounded-lg ${ditherCls} ${className}`} style={style}>
      {children}
    </div>
  )
}
 ...
 * 置信度图例 — 展示四级纹理含义
 */
export function ConfidenceLegend({ dark = false, className = '' }: { dark?: boolean; className?: string }) {
  const grades: ConfidenceGrade[] = ['very_high', 'high', 'medium', 'low']
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      {grades.map(g => {
        const cfg = CONFIDENCE_DITHER[g]
        const { style } = getDitherStyle(g, dark)
        return (
          <div key={g} className="flex items-center gap-1.5">
            <div
              className={`h-4 w-8 rounded-sm ${cfg.bg} ${cfg.border}`}
              style={style}
            />
            <div className="text-[9px]">
              <span className="font-medium text-gray-600 dark:text-gray-400">{cfg.label}</span>
              <span className="text-gray-400 ml-1">({scoreLabels[g]})</span>
            </div>
          </div>
        )
      })}
