...
export const acquisitionGrowthSolution: SolutionConfig = {
  ...
  workflows: [
    ...
    {
      id: 'deal_execution', name: '交易执行', icon: '✍️',
      description: '交易架构设计 → M&A 合同起草 → 税务架构方案 → 劳动关系处理',
      mode: 'sequential',
      estimatedTime: { human: '2 周', ai: '2 天' },
      aiRate: 60,
      deliverable: '交割文件包（交易架构方案 + M&A 合同 + 税务方案 + 员工安置方案）',
      successCriteria: ['交易架构含 ≥ 2 种方案对比', '合同含完整条款框架', '税务方案含节税金额测算'],
      steps: [
        { id: 'structure', agent: 'legal', expert: 'contract_reviewer', label: '交易架构设计',
          goal: '输出股权/资产收购架构方案对比', successCriteria: ['含法律风险对比', '含税务影响'],
          profitImpact: { dimension: 'cost_saving', amount: '最优架构节省税务成本 10-30%' } },
        { id: 'contract', agent: 'legal', expert: 'civil_lawyer', label: 'M&A 合同起草',
          goal: '起草完整收购协议框架（SPA/投资协议）', successCriteria: ['覆盖核心条款', '含风险分配条款'],
          profitImpact: { dimension: 'loss_avoidance', amount: '精准合同条款避免交割风险损失' } },
        { id: 'tax', agent: 'finance', expert: 'tax_consultant', label: '税务架构方案',
          goal: '设计交易税务最优路径', successCriteria: ['含印花税/增值税/所得税方案', '节税金额可量化'],
          profitImpact: { dimension: 'cost_saving', amount: '最优税务架构节省交易成本 5-15%' } },
        { id: 'hr', agent: 'hr', expert: 'hr_consultant', label: '员工安置方案',
          goal: '输出合规的员工留用/安置/补偿方案', successCriteria: ['符合劳动法 N/N+1/2N 要求', '含安抚沟通脚本'],
          profitImpact: { dimension: 'cost_saving', amount: '合规安置减少劳动纠纷损失 3-10 万' } },
      ],
      triggerPhrases: ['交易执行', 'M&A 合同', '并购签约', '交割'],
    },
    {
      id: 'integration_migration', name: '整合迁移', icon: '🔗',
      description: '账套迁移计划 → 服务切换 SOP → 客户通知 → 团队整合 → 系统并轨',
      mode: 'sequential',
      estimatedTime: { human: '1 个月', ai: '3 天' },
      aiRate: 55,
      deliverable: '整合迁移全套方案（迁移 SOP + 客户通知函 + 团队优化方案 + 验收清单）',
      successCriteria: ['客户流失率 ≤ 5%', '迁移 SOP 覆盖每个操作步骤', '员工安置方案合规'],
      steps: [
        { id: 'plan', agent: 'finance', expert: 'finance_accountant', label: '账套迁移计划',
          goal: '生成账套迁移 SOP 和时间表（最大化客户留存）',
          successCriteria: ['SOP 含逐步操作指导', '时间表分阶段推进', '风险应急预案'],
          profitImpact: { dimension: 'cost_saving', amount: '结构化迁移将客户流失从 30% 降至 5%' } },
        { id: 'notify', agent: 'growth', expert: 'content_creator', label: '客户通知与安抚',
          goal: '生成客户通知函、FAQ 和个性化安抚话术',
          successCriteria: ['通知函含明确服务延续承诺', 'FAQ 覆盖 Top 10 客户疑虑'],
          profitImpact: { dimension: 'revenue', amount: '精准安抚减少续费取消，年多留 ¥50-100 万' } },
        { id: 'team', agent: 'hr', expert: 'hr_consultant', label: '团队整合方案',
          goal: '输出人员保留/AI 替代/安置综合方案',
          successCriteria: ['明确每人去留方案', '补偿金额合规', '关键人才绑定方案'],
          profitImpact: { dimension: 'cost_saving', amount: 'AI 替代 60% 人力后年省 ¥50-100 万' } },
      ],
      triggerPhrases: ['整合迁移', '账套迁移', '客户迁移', '并购后整合'],
    },
    {
      id: 'client_activation', name: '存量客户激活', icon: '🔗',
      description: 'RFM 分群 → 交叉销售方案（法律/投资/HR） → 多渠道触达 → 转化跟踪',
      mode: 'sequential',
      estimatedTime: { human: '3 天', ai: '2 小时' },
      aiRate: 75,
      deliverable: 'RFM 分群报告 + 各群体交叉销售方案 + 多渠道触达文案',
      successCriteria: ['RFM 分群覆盖全部存量客户', '交叉销售方案含具体服务推荐', '单客价值提升 ≥ 30%'],
      steps: [
        { id: 'rfm', agent: 'growth', expert: 'growth_consultant', label: 'RFM 分群',
          goal: 'AI 对存量客户进行价值分群，识别交叉销售机会',
          successCriteria: ['分群 ≥ 3 类', '每类含价值描述和交叉机会'],
          profitImpact: { dimension: 'revenue', amount: '精准分群使交叉销售转化率翻倍' } },
        { id: 'cross', agent: 'sales', expert: 'sales_strategist', label: '交叉销售方案',
          goal: '为各群体设计法律/投资/HR 交叉销售包',
          successCriteria: ['每群体 ≥ 2 个推荐服务', '含价格锚点和成交话术'],
          profitImpact: { dimension: 'revenue', amount: '单客月收入 ¥300→¥800（+167%）' } },
        { id: 'outreach', agent: 'growth', expert: 'content_creator', label: '多渠道触达文案',
          goal: '生成邮件/短信/电话三渠道个性化触达文案',
          successCriteria: ['各渠道格式规范', '含个性化钩子（姓名/服务类型/时机）'],
          profitImpact: { dimension: 'revenue', amount: '个性化文案使回复率提升 3-5 倍' } },
      ],
      triggerPhrases: ['交叉销售', '存量激活', '客户增值', 'RFM'],
    },
  ],
  scenarios: [
