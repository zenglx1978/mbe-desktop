...
export const smbOperationsSolution: SolutionConfig = {
  ...
  slashCommands: [
    ...
    { cmd: '/月报', label: '月度经营体检', icon: '📊', workflowId: 'monthly_checkup' },
    { cmd: '/合同', label: '合同全流程', icon: '📝', workflowId: 'contract_lifecycle' },
    { cmd: '/辞退', label: '辞退方案', icon: '🚪', workflowId: 'employee_termination' },
    { cmd: '/记账报税', label: '月度记账报税', icon: '🧾', workflowId: 'monthly_accounting' },
    { cmd: '/合同审查', label: '合同签约审查', icon: '📋', workflowId: 'contract_review' },
    { cmd: '/客诉', label: '客诉升级处理', icon: '📢', workflowId: 'complaint_escalation' },
  ],
  safetyRules: [
  ...
  quickActions: [
    ...
    { id: 'contract-check', label: '合同全流程', icon: '📋', workflowId: 'contract_lifecycle', description: '上传合同，5 分钟获得风险审查+财务影响报告', cta: '上传审查' },
    { id: 'monthly-tax', label: '月度记账报税', icon: '🧾', workflowId: 'monthly_accounting', description: '一键汇总票据、计算税额、生成申报表', cta: '开始报税' },
    { id: 'fire-plan', label: '辞退方案', icon: '🚪', workflowId: 'employee_termination', description: '输入员工情况，AI 给出合法辞退方案+补偿金额', cta: '生成方案' },
    { id: 'complaint-handle', label: '客诉处理', icon: '📢', workflowId: 'complaint_escalation', description: '录入投诉，AI 评估严重度并给出话术+解决方案', cta: '处理投诉' },
  ],
  workflows: [
    {
    ...
    {
      id: 'contract_review', name: '合同签约审查', icon: '📋',
      description: '要素完整性检查 → 高风险条款识别 → 财税影响分析 → 修改建议（3天→5分钟）',
      mode: 'sequential',
      estimatedTime: { human: '3 天', ai: '5 分钟' },
      aiRate: 0.85,
      deliverable: '合同风险报告（逐条标注 + 修改建议 + 财税影响）',
      steps: [
        { id: 'completeness', agent: 'legal', expert: 'civil_lawyer', label: '合同要素完整性检查' },
        { id: 'risk_scan', agent: 'legal', expert: 'civil_lawyer', label: '高风险条款识别' },
        { id: 'tax_impact', agent: 'finance', expert: 'tax_consultant', label: '财税影响分析' },
        { id: 'suggestions', agent: 'legal', expert: 'civil_lawyer', label: '修改建议与替代条款' },
      ],
      triggerPhrases: ['合同快审', '合同条款', '审查合同风险'],
    },
    {
      id: 'monthly_accounting', name: '月度记账报税', icon: '🧾',
      description: '票据归集 → 凭证生成 → 税额计算 → 申报表生成（2天→30分钟）',
      mode: 'sequential',
      estimatedTime: { human: '2 天', ai: '30 分钟' },
      aiRate: 0.80,
      deliverable: '月度记账报表 + 税务申报表（可直接提交）',
      steps: [
        { id: 'collect', agent: 'finance', expert: 'finance_accountant', label: '票据归集与分类' },
        { id: 'voucher', agent: 'finance', expert: 'finance_accountant', label: '凭证生成' },
        { id: 'tax_calc', agent: 'finance', expert: 'tax_consultant', label: '税额计算' },
        { id: 'declaration', agent: 'finance', expert: 'tax_consultant', label: '申报表生成' },
      ],
      triggerPhrases: ['记账报税', '月度报税', '做账', '申报表'],
    },
    {
      id: 'employee_termination', name: '员工辞退方案', icon: '🚪',
      description: '合法性评估 → N/2N 补偿金计算 → 操作流程建议（2天→15分钟）',
      mode: 'parallel',
      estimatedTime: { human: '2 天', ai: '15 分钟' },
      aiRate: 0.70,
      deliverable: '合法辞退方案（法律依据 + 补偿金额 + 操作步骤）',
      steps: [
        { id: 'legality', agent: 'legal', expert: 'civil_lawyer', label: '辞退合法性评估' },
        { id: 'compensation', agent: 'legal', expert: 'civil_lawyer', label: '补偿金额计算（N/2N）' },
        { id: 'process', agent: 'legal', expert: 'civil_lawyer', label: '操作流程建议' },
      ],
      triggerPhrases: ['辞退员工', '解除劳动合同', '违法辞退', '补偿金'],
    },
    {
      id: 'complaint_escalation', name: '客诉升级处理', icon: '📢',
      description: '投诉分类 → 法律风险分析 → 话术与解决方案 → 老板确认（1天→20分钟）',
      mode: 'sequential',
      estimatedTime: { human: '1 天', ai: '20 分钟' },
      aiRate: 0.75,
      deliverable: '客诉处理方案（严重度评级 + 话术模板 + 法律风险说明）',
      steps: [
        { id: 'classify', agent: 'cs', expert: 'cs_consultant', label: '投诉分类与严重度评估' },
        { id: 'legal_risk', agent: 'legal', expert: 'civil_lawyer', label: '法律风险分析' },
        { id: 'solution', agent: 'cs', expert: 'cs_consultant', label: '话术与解决方案生成' },
      ],
      triggerPhrases: ['客户投诉', '12315', '客诉升级', '投诉处理'],
    },
  ],
  scenarios: [
